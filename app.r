                         ...
                         tabPanel("Réseau",
                                  sidebarLayout(
                                    ...
                                    sidebarPanel(# 1.2. Choix de la  variable à prédire :
                                      ...
                                      helpText("Attention, si vous faites varier la couleur ou l'épaisseur des liens seul l'argument 'solid' peut être utilisé."),
                                      selectizeInput("linetype", label="Forme des liens", choices=list("solid", "dotted", "dashed", "longdash", "twodash", "dotdash", "blank"), selected="solid")
                                      #)
                                    ),
                         ...
                         tabPanel("Ego-Réseau",
                                  sidebarLayout(
                                    ...
                                    sidebarPanel(# 1.2. Choix de la  variable à prédire :
                                      ...
                                      helpText("Attention, si vous faites varier la couleur ou l'épaisseur des liens seul l'argument 'solid' peut être utilisé."),
                                      selectizeInput("egolinetype", label="Forme des liens", choices=list("solid", "dotted", "dashed", "longdash", "twodash", "dotdash", "blank"), selected="solid")
                                    ),
                         ...
                         tabPanel("Sous-Réseau",
                                  sidebarLayout(
                                    ...
                                    sidebarPanel(# 1.2. Choix de la  variable à prédire :
                                      ...
                                      helpText("Attention, si vous faites varier la couleur ou l'épaisseur des liens seul l'argument 'solid' peut être utilisé."),
                                      selectizeInput("souslinetype", label="Forme des liens", choices=list("solid", "dotted", "dashed", "longdash", "twodash", "dotdash", "blank"), selected="solid")
                                    ),
                         ...
                         tabPanel("Sous réseau des communautés",
                                  sidebarLayout(
                                    ...
                                    sidebarPanel(# 1.2. Choix de la  variable à prédire :
                                      ...
                                      helpText("Attention, si vous faites varier la couleur ou l'épaisseur des liens seul l'argument 'solid' peut être utilisé."),
                                      selectizeInput("comlinetype", label="Forme des liens", choices=list("solid", "dotted", "dashed", "longdash", "twodash", "dotdash", "blank"), selected="solid")
                                    ),
...
server <- function(input, output, session) {
  ...
  GoT_union_nodes <- read_csv("~/zPublish/shiny/Reseau/data/GoT_union_nodes.csv")
  GoT_union_edges <- read_csv("~/zPublish/shiny/Reseau/data/GoT_union_edges.csv")
  ...
  output$downloadex2liens <- downloadHandler(
    filename = function() {
      paste("GoT_union_edges", ".csv", sep = "")
    },
    content = function(file) {
      write.csv(GoT_union_edges, file, row.names = FALSE)
    }
  )
  
  output$downloadex2noeuds <- downloadHandler(
    filename = function() {
      paste("GoT_union_nodes", ".csv", sep = "")
    },
    content = function(file) {
      write.csv(GoT_union_nodes, file, row.names = FALSE)
    }
  )
  ...
  output$Reseau <- renderPlot({ 
    ...
    ggraph(Mon_reseau(), layout = input$select_layout) + 
      geom_edge_link2(aes(edge_colour = get(col_edge()), edge_width = get(sizeedge())), edge_linetype= input$linetype, arrow = arrowedge() ) +
  ...
  output$egoreseau <- renderPlot({ 
    ...
    ggraph(monego(), layout = input$egoselect_layout) + 
      geom_edge_link2(aes(edge_colour = get(egocol_edge()), edge_width = get(egosizeedge())), edge_linetype= input$egolinetype, arrow = egoarrowedge()) +
  ...
  output$sousreseau <- renderPlot({ 
    ...
    ggraph(sousnet(), layout = input$sousselect_layout) + 
      geom_edge_link2(aes(edge_colour = get(souscol_edge()),edge_width = get(soussizeedge())), edge_linetype= input$souslinetype, arrow = sousarrowedge()) +
  ...
  output$comreseau <- renderPlot({ 
    ...
    ggraph(comnet(), layout = input$comselect_layout) + 
      geom_edge_link2(aes(edge_colour = get(comcol_edge()),edge_width = get(comsizeedge())), edge_linetype= input$comlinetype, arrow = comarrowedge()) +
