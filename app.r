                         ...
                         tabPanel("Réseau",
                                  sidebarLayout(
                                    ...
                                    sidebarPanel(# 1.2. Choix de la  variable à prédire :
                                      ...
                                      selectizeInput("couleuredges", label="Définition de la couleur des liens", choices=list("Couleur par défaut","Attribut noeuds"), selected=""),
                                      conditionalPanel(condition = "input.couleuredges=='Attribut noeuds'",
                         ...
                         tabPanel("Ego-Réseau",
                                  sidebarLayout(
                                    ...
                                    sidebarPanel(# 1.2. Choix de la  variable à prédire :
                                      ...
                                      selectizeInput("egocouleuredges", label="Définition de la couleur des liens", choices=list("Couleur par défaut","Attribut noeuds"), selected=""),
                                      conditionalPanel(condition = "input.egocouleuredges=='Attribut noeuds'",
                         ...
                         tabPanel("Sous-Réseau",
                                  sidebarLayout(
                                    ...
                                    sidebarPanel(# 1.2. Choix de la  variable à prédire :
                                      ...
                                      selectizeInput("souscouleuredges", label="Définition de la couleur des liens", choices=list("Couleur par défaut","Attribut noeuds"), selected=""),
                                      conditionalPanel(condition = "input.souscouleuredges=='Attribut noeuds'",
                         ...
                         tabPanel("Sous réseau des communautés",
                                  sidebarLayout(
                                    ...
                                    sidebarPanel(# 1.2. Choix de la  variable à prédire :
                                      ...
                                      selectizeInput("comcouleuredges", label="Définition de la couleur des liens", choices=list("Couleur par défaut","Attribut noeuds"), selected=""),
                                      conditionalPanel(condition = "input.comcouleuredges=='Attribut noeuds'",
...
server <- function(input, output, session) {
  ...
  output$Reseau <- renderPlot({ 
    ...
    ggraph(Mon_reseau(), layout = input$select_layout) + 
      geom_edge_link2(aes(edge_colour = get(col_edge()), edge_width = sizeedge()), arrow = arrowedge() ) +
  ...
  output$egoreseau <- renderPlot({ 
    ...
    ggraph(monego(), layout = input$egoselect_layout) + 
      geom_edge_link2(aes(edge_colour = get(egocol_edge()), edge_width = egosizeedge()), arrow = egoarrowedge()) +
  ...
  output$sousreseau <- renderPlot({ 
    ...
    ggraph(sousnet(), layout = input$sousselect_layout) + 
      geom_edge_link2(aes(edge_colour = get(souscol_edge()),edge_width = soussizeedge()), arrow = sousarrowedge()) +
  ...
  comsizeedge <- reactive({
    if(input$comsizeedge=="Pas de différence d'épaisseur")
      ...
      comsizeedge <- edge_attr(comnet())[[input$comsizeedge]]
  })
  ##
  ...
  output$comreseau <- renderPlot({ 
    ...
    ggraph(comnet(), layout = input$comselect_layout) + 
      geom_edge_link2(aes(edge_colour = get(comcol_edge()),edge_width = comsizeedge()), arrow = comarrowedge()) +
